...
* `com.cloudbees.jenkins.plugins.bitbucket.hooks.PushHookProcessor.scanOnPushWithEmptyChanges=false` for Cloud and Server (non-native)
* `com.cloudbees.jenkins.plugins.bitbucket.hooks.NativeServerPushHookProcessor.scanOnPushWithEmptyChanges=false` for Server native implementation

This properties must be set on Jenkins startup.

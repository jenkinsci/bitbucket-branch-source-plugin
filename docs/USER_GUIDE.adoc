...
_Manage Jenkins_ » _System_. Select one fallback credential. The credential selected by a job
remains the primary credential. When Bitbucket Cloud returns HTTP 429, the current request is
retried with the fallback credential. When Bitbucket sends `X-RateLimit-NearLimit: true`, the next
request starts with the fallback credential instead.

Fallback credentials must be valid Bitbucket Cloud API credentials visible from the Jenkins
system context and must use the same authentication method as the primary credential. Atlassian applies a
